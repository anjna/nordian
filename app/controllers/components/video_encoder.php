...
class VideoEncoderComponent extends Object {
	...
	function convert_videomp4 ($in, $out, $width = 480, $height = 360, $optimized = false) {		
		echo $command  =  Configure::read('Video.ffmpeg_path') . " -i {$in} -strict experimental -vcodec libx264 -acodec aac -ar 48000 -ac 2 -y {$out}";
		die;
		echo exec($command);
	}
	
	
	function convert_audio ($in, $out, $width = 480, $height = 360, $optimized = false) {
		$command = Configure::read('Video.ffmpeg_path') . " -i {$in} -b 256k -ac 1 -ar 44100 {$out}";
		echo exec($command);
	}
	...
	function grab_image ($in, $out) {
		...
		$command = Configure::read('Video.ffmpeg_path') . " -i {$in} -an -ss 00:00:01 -t 00:00:03 {$out}";
		shell_exec($command);
	}
	
	function get_duration ($in) {
		$command = Configure::read('Video.ffmpeg_path') . " -i {$in} 2>&1";
