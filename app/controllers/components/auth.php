 ...
 */
	function login($data = null) {
        ...
        //pr($this->data); die;
        if(isset($this->data['Admin']['keepmeloggedin']) && $this->data['Admin']['keepmeloggedin'] == 1){
            $cookie = array();
            $cookie['email'] = $this->data['Admin']['email'];
            $cookie['password'] = Security::hash($this->userdefinedPassword,null,true);
			$cookie['keepmeloggedin'] = $this->data['Admin']['keepmeloggedin'];
            $this->Cookie->write('Auth.Keepme', $cookie, false, '+2 weeks');
        }
        else if(isset($this->data['Admin']['keepmeloggedin'])){            
		...
		if ($user = $this->identify($data)) {
			if(@$user['is_verified']==0){
                $msgString = '<div class="failureMessage">Your account is not verified, please click on the link in email sent to you.</div>'; 
                $this->Session->setFlash($msgString);
            }elseif(@$user['is_active']==0){
                $msgString = '<div class="failureMessage">Your account has been deactivated by administrtor, please contact administrator.</div>'; 
                $this->Session->setFlash($msgString);
            }else{
        ...
        if($this->_loggedIn=='' && $msgString==''){
		    $msgString = '<div class="failureMessage">Email and Password does not match</div>'; 
		    $this->Session->setFlash($msgString);
        }
 ...
 */
	function hashPasswords($data) {
        ...
        $this->userdefinedPassword = @$data['Admin']['password'];
