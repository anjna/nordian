<?php
	...
	class UploadComponent extends Object {
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
			...
			if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('jpg','jpeg','gif','png','swf','JPG','PNG','GIF','JPEG'); }
			...
			if (isset($file) && is_array($file) && !$this->upload_error($file['error'])) {
              ...
              // -- just upload it
							...
							if (function_exists("imagecreatefromjpeg")) {
								...
								//pr($rules['type']); echo $rules['output'] . '-----' . $rules['quality']; pr($this->_file); echo $this->ext($fileName); exit;
