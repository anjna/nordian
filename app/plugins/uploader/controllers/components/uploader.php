...
class UploaderComponent extends Object {
	...
	public function initialize($Controller, array $settings = array()) {
		...
		//$this->set('maxFileSize',$maxFileSize);
	}
	...
	public function checkMimeType($ext, $type) {
           ...
           /*
		    if($ext == 'flv' && $type == 'application/octet-stream'){
                return true;
            }*/
	...
	protected function _returnData($data = '', $append = '', $explicit = false) {
		if (!empty($data) && !empty($append)) {
			...
			if ($explicit) {
				$img = explode(".",$this->_data[$this->_current]['name']);
				$name = $this->_name; #$img[0]."_".$data['width']."x".$data['height'].".".$img[1];
				return array(
					'path' => $path,
					'width' => $data['width'],
					'height' => $data['height'],
